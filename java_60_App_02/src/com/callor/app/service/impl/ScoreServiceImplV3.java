...
public class ScoreServiceImplV3 extends ScoreServiceImplV2{
	...
	public void saveScore() {
		 ...
		 * 
		 * 이때, 중간에 BufferedOutput는 파이프를 연결해두고
		 * 애플리케이션에서는 BufferedOutput에게 데이터를 모두 보내고 다른일을 수행한다.
		...
		try {
			...
			/*
			 * 아래 3개가 다 있어야 데이터가 저장됨
			 */
			//buffer.flush 데이터를 다 줬으니 알아서 처리해라
			buffer.flush();
			buffer.close();
			fileOut.close();
